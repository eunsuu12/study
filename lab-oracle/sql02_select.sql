...
-- DQL(Data Query Langtuage):
--  데이터베이스 테이블에서 레코드들을 검색하는 명령어.
...
-- 연결 연산자(||):
--  2개 이상의 컬럼을 합쳐서 하나의 컬럼으로 출력.
-- SQL에서 문자열은 작은따옴표('')를 사용함!
select deptno||' '||dname as "부서 정보"
from dept;

-- 부서(dept)테이블에서 부서이름(dname)과 위치(loc) 검색.
-- 출력 양식 예: ACCOUNT 부서는 NEW YORK에 있습니다.
...
--  정렬의 기본은 오름차순. asc는 생략 가능. 내림차순 정렬에서는 desc 명시.
...
-- 직원테이블(emp)에서 사번(empno)과 이름(ename)을 검색하고,
--  사번 오름차순 출력.
select empno, ename from emp order by empno;

-- 직원테이블에서 사번과 이름을 검색, 이름 오름차순 출력.
