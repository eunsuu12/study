...
3. DML(Data Manipulation Language): 데이터 조작 언어.
    ...
    - 테이블에 행(row)를 추가, 변경, 삭제하는 SQL 문장.
4. TCL(Transaction Control Language): 트랜잭션 관리(제어) 언어.
    - commit(변경 내용을 DB에 영구 저장), rollback(직전 commiit까지 상태로 되돌리기)
*/
...
--> 에러 발생: values(sid,sname,birthday) 순서인데, sid위치에 숫자 타입이 아닌 문자열을 전달했기 때문에.
...
-- create-as select 문장: 다른테이블에서 select한 내용 그대로 새로운 테이블을 생성.
