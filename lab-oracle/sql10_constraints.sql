-- # 제약 조건(constraints)
-- 테이블에 데이터들이 insert 될 때, 조건을 만족하는 값들만 insert될 수 있도록 만들어 주는 기능.
...
--> primary key(고유키)= not null + unique(중복되지 않는 값)
...
--> 에러 발생: col_3은 unique 제약 조건인데, 기존 입력 값과 같은 'test'를 insert하려고 했기 때문에.
...
--> 에러 발생: col_4는 >= 0 (check 제약 조건)이어야 하는데, -1을 insert하려고 했기 때문에.
...
create table ex03(
    ...
    constraint ex04_col4_ck check (col4 in('M','F'))
);

-- foreign key(외래키): 다른 테이블의 primary key를 참조하는 제약 조건.
-- 데이터를 삽입(insert)할 때 다른 테이블의 PK에 없는 값은 저장되지 않도록 하기 위해 사용.
